...
function changeMode(mode)
{
  ...
  currentMode=mode;
  $('#lyricsButton').removeClass('buttonPressed'); $('#artworkButton').removeClass('buttonPressed'); $('#youtubeButton').removeClass('buttonPressed');
  switch(mode)
  {
  case "lyrics":
...
{  
  ...
  else if(pageLyrics.is(':visible')) {var toHide=pageLyrics;}
  else{var toHide=pageYoutube;}
  ...
  switch(command)
  {
    case "delete":
     $.ajax({ url : "/syncServer.php"+"?command=delete&id="+id+"&index="+arg,    type: "GET",
      ...
      {
        renewPlaylist();
        if(data=="not_loggedIn"){alert("You are not logged in"); window.location.replace("/");}
      }  
            });
     break;
    case "addNewPS":
...
function setCurrentPlaylist(playlistReceived)
{
  switch(playlistReceived)
  {
    case "All Music":
      ...
      $(".playlist").empty();
      fileData=JSON.parse(JSON.stringify(fileDataBackUp)); playlist(); feedAutoComplete();
    break;
    case "VK":
      currentPlaylist="VK";
      //show VK search enabler
      $("#VKSearchEnabler").show(); 
      if(toggleVKSearchEnabled){toggleVKSearchEnabler();}
      $("#tags").addClass("tagswithVK");
      //show VK search enabler
      fileData.url=new Array();fileData.title=new Array();fileData.artist=new Array();fileData.album=new Array();fileData.year=new Array();fileData.urlOfArt=new Array();fileData.genre=new Array();fileData.wave=new Array();fileData.id=new Array();
...
{
    $(".dropdown").on('changed', function(e, indexPl, name)
       {//alert(name);
        ...
        else
        {
          ...
          {
             $(".playlist > div").hide(); fileData={}; feedAutoComplete();
          } //EMPTY PLAYLIST
          else{//alert("pressed");
                fileData=JSON.parse(JSON.stringify(fileDataBackUp)); //non empty
                console.log("----------");console.log(fileData.id);console.log("\n");
  ...
  function changeTheme(customTheme, type)
  {
    ...
    if (type=="png"){filetype=".png"}
      else if (type=="jpg"){filetype=".jpg"}
        else if (type=="gif"){filetype=".gif"}
          else {filetype=".svg"}
  }
  ...
  function setCurrentFile(action)
  {
    if (action=="prev")
    {
      ...
      fileNumber--;
      if (fileNumber<0) {fileNumber=lengthOfJsonObject-1};
      
      changeSongBack(fileNumber);

      $(".playlist").scrollTo( ('#song'+fileNumber), 200, {easing:'linear'} );
    ...
    else if (action=="next")
    {
      ...
      fileNumber++;
      if (fileNumber>=lengthOfJsonObject) {fileNumber=0;};
...
//nonfunction------------------------------------------------------------------------------
  function keyboardControlSong(e) 
  {
    if (keyboardControl)
    {
      ...
      switch (e.which) 
      {
        case 32: currentFile.paused?playButton():pauseButton(); e.preventDefault() ;break; //space
        case 37: swapControl?prevSong():seek("backward", 10); e.preventDefault() ;break; //left
        case 38: volumePercent>=100?volumePercent:volumePercent=volumePercent+5;volume(volumePercent); e.preventDefault() ; break; //up
        case 39: swapControl?nextSong():seek("forward", 10); e.preventDefault() ;break; //right
        case 40: volumePercent<=0?volumePercent:volumePercent=volumePercent-5;volume(volumePercent); e.preventDefault() ; break; //down

        case 188: swapControl==false?prevSong():seek("backward", 10); e.preventDefault() ;break; //left
        case 190: swapControl==false?nextSong():seek("forward", 10); e.preventDefault() ;break; //right
  ...
  function progressUpdate() 
  {
    if(currentFile.buffered.length > 0) 
    {
      ...
      $(controls.buffered).css({ width: percent + "%" });
    }
  }
  ...
  function timeupdateSong()
  {
    if (!isBeingAnimated) {
        if (seeking==false)
        {
          ...
          $(controls.played).css({ width: timePercent + "%" });
        }
      }
    if (controls.timeProgress !=null || controls.timeProgress !=undefined) 
      {
...
//================================================================================================
  function startTimer(value, container) 
  {
    timerCount=value*60;
    var obj={};
    if (container!=undefined || container!=null) {$(container).text((Math.floor(timerCount/60)<10?"0"+(Math.floor(timerCount/60)):Math.floor(timerCount/60))+":"+((timerCount-Math.floor(timerCount/60)*60)<10?"0"+(timerCount-Math.floor(timerCount/60)*60):timerCount-Math.floor(timerCount/60)*60));}
    timer=setInterval(function(){timerDecrease(container)},1000);
  }
...
//buffered---------------------------------------------
  ...
  { 
    ...
    $(controls.indicator).offset({left: e.pageX-($(controls.indicator).width()/2)});
  ...
  function seekbarMouseUp(e)
  {
    ...
    try{currentFile.currentTime=(relX*currentFile.duration)/100;}
      catch(e){console.log("Please wait until music is loaded");}
    //------------------------------------------------------------------------
  ...
  function seekbarMouseMove(e)
  {
    ...
    $(controls.indicator).offset({left: ind});
    $(controls.played).css({ width: relX + "%" });
  }
//volume--------------------------------------------- 
  ...
  { 
    ...
    $(controls.volumeIndicator).offset({left: e.pageX-($(controls.volumeIndicator).width()/2)});
  ...
  function volumeMouseMove(e)
  {
    ...
    $(controls.volumeIndicator).offset({left: ind});
...
//functionality---------------------------------------------
  function volume(vol) 
  {
    ...
    volumePercent=vol;
    $(controls.volumeLevel).css({ width: vol + "%" });
  ...
  function repeatSong()
  {
    ...
    if (repeat==0){$(".repeat > img").replaceWith("<img src='"+defaultTheme+theme.repeatImg+filetype+"' />");}
    else if (repeat==1){$(".repeat > img").replaceWith("<img src='"+defaultTheme+theme.repeatOneImg+filetype+"' />");}
    else {$(".repeat > img").replaceWith("<img src='"+defaultTheme+theme.repeatAllImg+filetype+"' />");}
  ...
  function pauseButton()
  {
    ...
    currentFile.pause();
    $("#playButton").show(); $("#pauseButton").hide();
